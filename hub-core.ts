...
const HUB_CLIENT_VERSION = "2.1.0";
...
function hubHome(): string {
	...
	if (process.env.PI_HOME) return join(process.env.PI_HOME, "hub-dashboard");
	return join(homedir(), ".hub-dashboard");
}

function hubDir(): string {
	return join(hubHome(), "server");
}
...
function serverAutoStartAllowed(config: HubConfig): boolean {
	return config.autoStartServer && !isServerManuallyStopped();
}

function serverBaseUrl(config: HubConfig): string {
	const host = config.host === "0.0.0.0" ? "127.0.0.1" : config.host;
	return `http://${host}:${config.port}`;
}

function getServerScript(): string {
	return join(__dirname, "hub-server.mjs");
}
...
async function waitForServer(config: HubConfig, params: HubParams, timeoutMs = 5000): Promise<void> {
...
async function ensureServer(config: HubConfig, params: HubParams, waitMs = 7000): Promise<void> {
	...
	if (pid && isProcessRunning(pid)) {
		await waitForServer(config, params, 3000);
		return;
	}

	if (!config.autoStartServer) {
		throw new Error(`${params.displayName} server is not running and autoStartServer=false`);
	}
...
async function getJson(config: HubConfig, path: string): Promise<any> {
...
function clientMetadata(params: HubParams) {
	return {
...
function slashCommandSummaries(api: any): Array<{ name: string; description?: string; argumentCompletions?: Array<{ value: string; label?: string }> }> {
	try {
		return api.getCommands().map((command: any) => {
...
function currentSessionInfo(ctx: ExtensionContext, config: HubConfig, status: string, params: HubParams, slashCommands: ReturnType<typeof slashCommandSummaries> = [], availableModels: ReturnType<typeof availableModelSummaries> = []) {
	return {
...
function networkHint(config: HubConfig): string {
...
function firewallHint(config: HubConfig, params: HubParams): string {
	const port = Number(config.port);
	if (process.platform === "win32") {
		return [
			"Windows Firewall (run PowerShell as Administrator):",
			`New-NetFirewallRule -DisplayName "${params.displayName} ${port}" -Direction Inbound -Action Allow -Protocol TCP -LocalPort ${port}`,
			"",
			"Remove rule:",
			`Remove-NetFirewallRule -DisplayName "${params.displayName} ${port}"`,
		].join("\n");
	}
	...
	return [
		...
		`sudo ufw allow ${port}/tcp comment '${params.displayName}'`,
...
async function hubStatusText(config: HubConfig, connected: boolean, params: HubParams): Promise<string> {
	const pid = readPid();
...
function hubInfoText(config: HubConfig, params: HubParams): string {
	return [
		`=== ${params.displayName} Info ===`,
		`Config: ${configPath()}`,
		`PID file: ${pidPath()}`,
		"",
		networkHint(config),
...
export function createHubExtension(api: any, params: HubParams) {
	...
	function sessionSlashCommands(): ReturnType<typeof slashCommandSummaries> {
		cachedSlashCommands ??= slashCommandSummaries(api);
	...
	if (name === "hub") {
		...
		if (sub === "start") {
			...
			await register(ctx);
			notifyFallback(`${params.displayName} started.\n\n${networkHint(config)}`, "info");
		} else if (sub === "stop") {
			await disconnectSession();
			notifyFallback(`Disconnected this session from ${params.displayName}. Server still running for other sessions.`, "info");
			...
			notifyFallback(hubInfoText(config, params), "info");
		} else if (sub === "status") {
			const text = await hubStatusText(config, serverOk, params);
			notifyFallback(text, text.includes("Server HTTP: reachable") ? "info" : "warning");
		} else if (sub === "firewall") {
			notifyFallback(firewallHint(config, params), "info");
		} else {
		...
		throw new Error(`Remote execution for /${name} is not supported yet. Use the ${params.tuiName} for this slash command.`);
	}
...
async function handleCollaborationMessage(command: any): Promise<void> {
				...
				try {
					...
					if (command?.type === "user_message") {
						...
						const sendOptions = deliveryMode === "followUp" || (deliveryMode === "auto" && sameSessionBusy)
							? { deliverAs: "followUp" }
							: undefined;
						await Promise.resolve(api.sendUserMessage(content, sendOptions));
						await sendEvent({
							type: "input",
							item: {
					...
					if (command?.type === "set_model") {
						...
						await Promise.resolve(api.setModel(model));
						await sendCommandResult(command, true);
						continue;
					}
	...
	api.registerCommand("hub", {
		description: `${params.displayName}: /hub [info|status|firewall|start|stop|server stop]`,
		...
		async handler(args, ctx) {
			...
			if (sub === "start") {
				try {
					...
					startBackgroundLoops();
					await register(ctx);
					ctx.ui.notify(`${params.displayName} started.\n\n${networkHint(config)}`, "info");
				} catch (error) {
					ctx.ui.notify(`${params.displayName} start failed: ${error instanceof Error ? error.message : String(error)}`, "error");
				}
				return;
			}
			if (sub === "stop") {
				if (!serverOk) {
					ctx.ui.notify(`This session is not connected to ${params.displayName}.`, "warning");
					return;
				}
				await disconnectSession();
				ctx.ui.notify(`Disconnected this session from ${params.displayName}. Server still running for other sessions.\nUse /hub start to reconnect, or /hub server stop to kill the server.`, "info");
				return;
			}
			if (sub === "server stop") {
				markServerManuallyStopped();
				...
				if (pid && isProcessRunning(pid)) {
					...
					try {
						process.kill(pid);
						ctx.ui.notify(`${params.displayName} server killed (PID ${pid}). Auto-restart disabled until /hub start.`, "info");
					} catch (error) {
						ctx.ui.notify(`Failed to kill server (PID ${pid}): ${error instanceof Error ? error.message : String(error)}`, "error");
					}
				} else {
					await disconnectSession();
					ctx.ui.notify(`${params.displayName} server is not running. Auto-restart disabled until /hub start.`, "warning");
				}
				return;
			}
			if (sub === "info" || !sub) {
				ctx.ui.notify(hubInfoText(config, params), "info");
				return;
			}
			if (sub === "status") {
				const text = await hubStatusText(config, serverOk, params);
			...
			if (sub === "firewall") {
				ctx.ui.notify(firewallHint(config, params), "info");
				return;
			}
