...
function hubHome() {
  ...
  if (process.env.PI_CODING_AGENT_DIR) return path.join(process.env.PI_CODING_AGENT_DIR, "hub-dashboard");
  if (process.env.PI_HOME) return path.join(process.env.PI_HOME, "hub-dashboard");
  return path.join(os.homedir(), ".hub-dashboard");
}

const HUB_DIR = path.join(hubHome(), "server");
...
function normalizeAgentCreationConfig(value = {}) {
  ...
  const commands = Object.fromEntries(
    Object.entries({ ...DEFAULT_CONFIG.agentCreation.commands, ...rawCommands })
...
function detectAvailableClis() {
  if (Date.now() - _cliCache.at < 30_000) return _cliCache.list;
  const list = Object.entries(config.agentCreation.commands)
...
function validateAgentCreationRequest(body = {}) {
  ...
  const requestedCli = creationText(body.cli, "cli", 80) || config.agentCreation.defaultCli;
  if (!config.agentCreation.commands[requestedCli]) throw new Error(`unknown cli: ${requestedCli}`);
  return {
    cli: requestedCli,
    cwd,
...
function startAgentCreation(request) {
  const createdAt = Date.now();
  const cliCommand = config.agentCreation.commands[request.cli];
  if (!cliCommand) throw new Error(`unknown cli: ${request.cli}`);
  const creation = {
  ...
  const env = {
    ...process.env,
    HUB_AGENT_CLI: request.cli,
    HUB_AGENT_CWD: request.cwd,
    HUB_AGENT_NAME: request.name,
    HUB_AGENT_MODEL: request.model,
    HUB_INITIAL_PROMPT: request.initialPrompt,
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === "POST" && url.pathname === "/api/send-attachment") {
      ...
      const attachmentDir = path.join(os.tmpdir(), "hub-dashboard-attachments");
